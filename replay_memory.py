...
class ReplayMemory:
  def __init__(self, size, dimO, dimA, dtype=np.float32):
    ...
    self.next_observations = np.empty(so, dtype = dtype)
  ...
  def enqueue(self, observation, action, reward, next_observation, terminal, info=None):
    
    self.i = (self.i + 1) % self.size
    
    self.observations[self.i, ...] = observation
    self.next_observations[self.i,...] = next_observation
  ...
  def minibatch(self,size):
    ...
    o2 = self.next_observations[indices,...]
